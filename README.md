...
### Pipelining
The pipelined server was built over top the Persistent server. Our aim in the pipelined server was to handle all the responses sent by a single client in parallel. So this means every request a single client sends is recieved, processed, and sent in parallel. To do this, after a client connection is recieved we create *MAX_REQUESTS* number of threads each of which runs the *processRequestByThread* function concurrently. The *processRequestByThread* function essentially just has everything from our Persistence server in reading, processing, and responding to requests. Another thing we had is a global *keep_alive* variable accessible by all threads and one which all threads modify (if *keep_alive* is set to 0, the connection closes). As such we needed to create locks (pthread_mutex_t) to avoid synchronization bugs, we have 3 locks: keep_alive_mutex, read_from_client_mutex, and write_to_client_mutex. The keep_alive_mutex is to avoid race conditions in the *keep_alive* variable, the read_from_client_mutex is so only one thread can read a request at a time, and write_to_client_mutex is so only one thread can write to the client at a time. In the *processRequestByThread* function, if the thread enters and sees that *keep_alive* is 0, it exits.
